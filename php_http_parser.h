...
#define php_http_parser_array_get_value(ht, str, v)                                                                    \
    ((v = zend_hash_str_find(ht, str, sizeof(str) - 1)) && !ZVAL_IS_NULL(v))

#define HTTP_PARSER_SET_CLASS_CREATE(module, _create_object) module##_ce->create_object = _create_object

#define HTTP_PARSER_SET_CLASS_FREE(module, _free_obj) module##_handlers.free_obj = _free_obj
...
#define HTTP_PARSER_CLASS_ALIAS(name, module)                                                                          \
    do {                                                                                                               \
        if (name) {                                                                                                    \
...
#define HTTP_PARSER_CLASS_ALIAS_SHORT_NAME(shortName, module)                                                          \
    do {                                                                                                               \
...
#define HTTP_PARSER_SET_CLASS_CLONEABLE(module, _clone_obj) module##_handlers.clone_obj = _clone_obj
...
#define HTTP_PARSER_INIT_CLASS_ENTRY_BASE(module, namespaceName, snake_name, shortName, methods, parent_ce)            \
    do {                                                                                                               \
...
#define HTTP_PARSER_INIT_CLASS_ENTRY_EX2(                                                                              \
    module, namespaceName, snake_name, shortName, methods, parent_module_ce, parent_module_handlers)                   \
...
static inline void http_parser_zend_class_unset_property_deny(zval *zobject, zval *zmember, void **cache_slot) {
    ...
    if (EXPECTED(zend_hash_find(&ce->properties_info, Z_STR_P(zmember)))) {
        zend_throw_error(NULL,
                         "Property %s of class %s cannot be unset",
                         Z_STRVAL_P(zmember),
                         HTTP_PARSER_Z_OBJCE_NAME_VAL_P(zobject));
        return;
    }
    std_object_handlers.unset_property(zobject, zmember, cache_slot);
}
#else
static inline void http_parser_zend_class_unset_property_deny(zend_object *object,
                                                              zend_string *member,
                                                              void **cache_slot) {
